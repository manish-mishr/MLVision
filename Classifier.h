...
public:
  ...
  virtual string classify(const string &filename,const string &clas,const Dataset &filenames) = 0;
  ...
  void test(const Dataset &filenames)
  {
      ...
      for(vector<string>::const_iterator f_iter = c_iter->second.begin(); f_iter != c_iter->second.end(); ++f_iter)
	{
	  ...
	  predictions[c_iter->first][*f_iter]=classify(*f_iter,c_iter->first,filenames);
	}
